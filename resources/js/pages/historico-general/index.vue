    ...
    return props.periods.map((period) => {
        ...
        return {
            ...
            report_final_available:
                ...
                false,
            radiography_status:
                (grouped as any)?.radiography_status ??
                (period as any).radiography_status ??
                'missing',
            radiography_invalidated: Boolean(
                (grouped as any)?.radiography_invalidated ??
                (period as any).radiography_invalidated ??
                false,
            ),
...
function radiographyBadge(period: PeriodRow) {
    ...
    if (period.radiography_status === 'generated')
        return {
            label: 'Lista',
...
async function submitRadiography(action: 'generar' | 'regenerar') {
    ...
    const result = await Swal.fire({
        title: `¿${action === 'generar' ? 'Generar' : 'Regenerar'} radiografía?`,
        text: 'Se consolidarán las métricas del periodo.',
        icon: 'question',
        showCancelButton: true,
    ...
    if (!result.isConfirmed) return;
    router.post(
        `/reportes-mensuales/${selectedPeriodRow.value.id}/consolidar`,
        {},
        { preserveScroll: true },
    );
}
    ...
    const renderProgress = async () => {
        try {
            const response = await fetch(
                `/historico-general/${uploadId}/progreso`,
                {
                    headers: {
            ...
            const totalKnown = Math.max(
                rowsRead,
                rowsInserted + rowsSkipped + rowsWithErrors,
                1,
            );
                    ...
                    >
                        <div class="border-b px-4 py-4 sm:px-5">
                            ...
                            >
                                <div>
                                    ...
                                    >
                                        Revisa, analiza o elimina archivos ya
                                        cargados.
                                    </p>
                                </div>
                        ...
                        >
                            ...
                            >
                                <button
                                    type="button"
                                    class="app-btn app-btn-primary h-11 px-4"
                                    @click="submitRadiography('generar')"
                                >
                                    Generar Radiografía
                                </button>
                                <button
                                    ...
                                    @click="submitRadiography('regenerar')"
                                >
                                    Regenerar Radiografía
                                </button>
                                <a
                                    :href="
                                        selectedPeriodRow
