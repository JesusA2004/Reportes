...
<title>Error en reprocesamiento — {{ $upload->dataSource?->name ?? 'Archivo' }}</title>
</head>
    ...
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,.08);">
      ...
      <tr>
        <td style="background:#0f172a;padding:28px 36px;">
          ...
          <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:900;line-height:1.3;">Error en reprocesamiento</h1>
        </td>
      </tr>
      ...
      <tr>
        <td style="background:#fee2e2;padding:14px 36px;border-bottom:1px solid #fecaca;">
          <p style="margin:0;color:#dc2626;font-size:13px;font-weight:700;">✗ &nbsp;El reprocesamiento terminó con error</p>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:36px 36px 28px;">
          ...
          <p style="margin:0 0 28px;color:#334155;font-size:15px;line-height:1.7;">
            El reprocesamiento del archivo <strong style="color:#0f172a;">{{ $upload->dataSource?->name ?? $upload->original_name }}</strong>
            del periodo <strong style="color:#0f172a;">{{ $period->label }}</strong> terminó con un error.
            Por favor revisa el archivo y vuelve a intentarlo desde Histórico General.
          </p>
          ...
          @if($errorMessage)
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#fff7ed;border:1px solid #fed7aa;border-radius:12px;overflow:hidden;margin-bottom:28px;">
            <tr>
              <td style="padding:16px 18px;">
                <p style="margin:0 0 6px;font-size:12px;font-weight:700;color:#c2410c;text-transform:uppercase;letter-spacing:.08em;">Detalle del error</p>
                <p style="margin:0;font-size:13px;color:#9a3412;font-family:monospace;word-break:break-all;">{{ $errorMessage }}</p>
              </td>
            </tr>
          </table>
          @endif
