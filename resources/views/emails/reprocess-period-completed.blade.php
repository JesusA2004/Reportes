...
<title>Reprocesamiento completado — {{ $period->label }}</title>
</head>
    ...
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,.08);">
      ...
      <tr>
        <td style="background:#0f172a;padding:28px 36px;">
          ...
          <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:900;line-height:1.3;">Reprocesamiento del periodo completado</h1>
        </td>
      </tr>
      ...
      <tr>
        <td style="background:#dcfce7;padding:14px 36px;border-bottom:1px solid #bbf7d0;">
          <p style="margin:0;color:#15803d;font-size:13px;font-weight:700;">✓ &nbsp;Todas las fuentes reprocesadas correctamente</p>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:36px 36px 28px;">
          ...
          <p style="margin:0 0 28px;color:#334155;font-size:15px;line-height:1.7;">
            El reprocesamiento completo del periodo <strong style="color:#0f172a;">{{ $period->label }}</strong>
            finalizó correctamente. Todos los datos fueron actualizados en la base de datos.
            Puedes ejecutar la actualización de BD y generar la Radiografía nuevamente.
          </p>
          ...
          @if(!empty($results))
          <p style="margin:0 0 12px;font-size:13px;font-weight:700;color:#334155;">Detalle por fuente</p>
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:12px;overflow:hidden;margin-bottom:28px;">
            ...
            <tr style="{{ !$loop->last ? 'border-bottom:1px solid #bfdbfe;' : '' }}">
              <td style="padding:12px 18px;font-size:13px;color:#1d4ed8;font-weight:600;width:55%;">{{ $result['name'] ?? $code }}</td>
              <td style="padding:12px 18px;font-size:13px;color:#1e40af;font-weight:700;">
                @if($result['ok'] ?? false)
                  {{ number_format($result['inserted'] ?? 0) }} registros
                @else
