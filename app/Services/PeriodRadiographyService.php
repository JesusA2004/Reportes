...
class PeriodRadiographyService
{
    ...
    public function generate(Period $period, ?int $userId = null, array $config = []): PeriodSummary
    {
        ...
        // Cartera/mora son un estado al corte, no un flujo — para un periodo automático
        // (bimestral/trimestral) deben salir SOLO del último mes componente, nunca sumados
        // entre meses (ver Period::resolveLastMonthlyComponent()).
        $closingDataIds = $dataIds;
        if ($period->isAutoGenerated()) {
            $lastMonthly = $period->resolveLastMonthlyComponent($allPeriods);
            if ($lastMonthly) {
                $closingWeekly = $lastMonthly->resolveBaseWeeklyIds($allPeriods);
                $closingDataIds = array_values(array_unique(array_merge(
                    empty($closingWeekly) ? [] : $closingWeekly,
                    [$lastMonthly->id]
                )));
            }
        }

        // Periodos automáticos: si algún mes componente no está generado, no consolidar
        // nada — evita un consolidado a medias (el chequeo de ReportUploadController ya
        // bloquea esto en el flujo normal, pero generate() puede invocarse desde otros
        // puntos, ej. MonthlyReportController::consolidate()).
        if ($period->isAutoGenerated() && !empty($period->component_period_ids)) {
            ...
            foreach ($allPeriods->whereIn('id', $componentIds) as $comp) {
                $compSummary = PeriodSummary::query()->where('period_id', $comp->id)->where('status', 'generated')->whereNull('invalidated_at')->first();
                if (!$compSummary) {
                    $missingMonths[] = $comp->label;
                }
            }
            if (!empty($missingMonths)) {
                throw new \RuntimeException('No se puede consolidar el periodo automático: falta generar el reporte mensual de: ' . implode(', ', $missingMonths) . '.');
            }
        }
        ...
        // Calcular ANTES de escribir nada — si un periodo automático consolida en $0 pese a
        // que sus meses componentes existen, es un error del motor de consolidación, no un
        // resultado real. No se debe guardar como generado ni dejar Excel/PDF en ceros.
        $gm = $this->globalMetrics($period, $dataIds, $includedBranchIds, $closingDataIds);
        if ($period->isAutoGenerated()) {
            $flowIsZero = (float) ($gm['recuperacion_total'] ?? 0) == 0.0
                && (float) ($gm['colocacion_total'] ?? 0) == 0.0
                && (float) ($gm['gasto_total'] ?? 0) == 0.0
                && (float) ($gm['gastos_nomina'] ?? 0) == 0.0;
            if ($flowIsZero) {
                throw new \RuntimeException(
                    'No se pudo consolidar el periodo automático: no se cargaron snapshots de meses componentes (todos los KPIs de flujo salieron en $0 pese a que los meses componentes existen). Generación detenida — no se guardó ningún reporte.'
                );
            }
        }

        return DB::transaction(function () use ($period, $userId, $uploads, $dataIds, $closingDataIds, $includedBranchIds, $gm) {
            ...
            foreach ($this->branchMetrics($period, $dataIds, $includedBranchIds, $closingDataIds) as $row) {
            ...
            PeriodCorporateSummary::query()->updateOrCreate(
                ['period_summary_id' => $summary->id],
                ['metrics' => $this->corporateMetrics($period, $dataIds, $includedBranchIds, $closingDataIds)],
            );
    ...
    private function globalMetrics(Period $period, array $dataIds, array $includedBranchIds = [], ?array $closingDataIds = null): array
    {
        ...
        $branchCalcResult = $this->branchCalculator->buildBranches($period, $dataIds, $includedBranchIds, $closingDataIds ?? $dataIds);
    ...
    private function corporateMetrics(Period $period, array $dataIds, array $includedBranchIds = [], ?array $closingDataIds = null): array
    {
        return $this->globalMetrics($period, $dataIds, $includedBranchIds, $closingDataIds);
    }

    private function branchMetrics(Period $period, array $dataIds, array $includedBranchIds = [], ?array $closingDataIds = null): array
    {
        $closingDataIds = $closingDataIds ?? $dataIds;
        $rows = [];
        ...
        $branchIds = collect()
            ...
            ->merge(Portfolio::query()->whereIn('period_id', $closingDataIds)->pluck('branch_id'))
            ->filter()
            ->unique()
        ...
        foreach ($branchIds as $branchId) {
            $valor = (float) Portfolio::query()
                ->whereIn('period_id', $closingDataIds)
