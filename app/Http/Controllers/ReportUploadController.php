...
class ReportUploadController extends Controller
{
    public function index(): Response
    {
        ...
        $summariesByPeriod = PeriodSummary::query()
            ->with('incidents')
            ->get()
            ->keyBy('period_id');
        ...
        $periods = $periodModels->map(function (Period $period) use ($weeklyPeriods, $sources, $requiredSourcesCount, $summariesByPeriod) {
            ...
            return [
                ...
                ...$this->resolveWorkflowState($uploads, $summary),
                'available_week_options' => $period->type === 'weekly'
                    ? $weeklyPeriods
            ...
            return [
                ...
                'uploads' => $uploads->unique('id')->values()->map(fn ($upload) => [
                    ...
                    'covered_period_labels' => collect($upload->covered_period_ids ?? [])
                        ->map(fn ($weekId) => optional($weeklyPeriods->firstWhere('id', (int) $weekId))->label)
                        ->filter()
                        ->values(),
                ]),
            ];
    ...
    public function store(StoreReportUploadRequest $request, ReportUploadService $service): RedirectResponse
    {
        $service->store(
            ...
            collect($request->input('covered_period_ids', []))
                ->map(fn ($id) => (int) $id)
                ->values()
                ->all(),
    ...
    {
        $coveredWeeks = $this->resolveCoveredWeeks(
            $period,
            Period::query()->where('type', 'weekly')->get(),
        );

        $uploads = $this->resolveUploadsForPeriod($coveredWeeks);
        $summary = PeriodSummary::query()->where('period_id', $period->id)->with('incidents')->first();
        $workflow = $this->resolveWorkflowState($uploads, $summary);

        if (!$workflow['can_generate_radiography']) {
            return back()->with(
                'error',
                implode(' ', $workflow['blocking_reasons']) ?: 'No se puede generar la Radiografía todavía.',
            );
        }
    ...
    private function resolveCoveredWeeks(Period $period, Collection $weeklyPeriods): Collection
    {
        ...
        return $weeklyPeriods
            ->filter(fn ($week) => $week->start_date
                && $week->end_date
                && $period->start_date
                && $period->end_date
                && $week->start_date->lte($period->end_date)
    ...
    private function resolveUploadsForPeriod(Collection $coveredWeeks): Collection
    {
        ...
        return ReportUpload::query()
            ...
            ->get()
            ->filter(fn (ReportUpload $upload) => collect($upload->covered_period_ids ?? [])
                ->map(fn ($id) => (int) $id)
                ->intersect($weekIds)
                ->isNotEmpty())
    ...
    private function resolveWorkflowState(Collection $uploads, ?PeriodSummary $summary): array
    {
        ...
        $missingDb = collect(['noi_nomina', 'lendus_ingresos_cobranza'])
            ->filter(fn ($code) => !$sourceCodes->contains($code))
            ->values()
            ->all();

        $missingRadiography = collect([
            'noi_nomina',
            'lendus_ingresos_cobranza',
            'gastos',
            'lendus_ministraciones',
            'lendus_saldos_cliente',
        ])->filter(fn ($code) => !$sourceCodes->contains($code))->values()->all();

        $pendingCritical = (int) ($summary?->incidents()->where('severity', 'high')->count() ?? 0);

        $databaseUpdated = in_array($summary?->status, ['database_updated', 'generated'], true)
        ...
        if (!empty($missingDb)) {
            $blockingReasons[] = 'No se puede actualizar la BD. Faltan archivos obligatorios: NOI Nómina, Lendus Ingresos Cobranza.';
        }

        if (!$databaseUpdated) {
            $blockingReasons[] = 'Primero actualiza la BD.';
        }

        if ($pendingCritical > 0) {
            $blockingReasons[] = 'Hay incidencias pendientes antes de generar la Radiografía.';
        }

        if (!empty($missingRadiography)) {
            $blockingReasons[] = 'Faltan fuentes para analizar y generar Radiografía.';
        }
